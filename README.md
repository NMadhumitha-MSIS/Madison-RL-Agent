...
This project integrates reinforcement learning into the Madison Intelligence Agent framework
from Humanitarians.AI. Madison agents autonomously gather, evaluate, and synthesize
information from multiple sources. The RL system learns which sources to trust for
...
### Agentic System Type
Research and Analysis Agents -- learning effective information gathering strategies

---

## Quick Start (Google Colab)

1. Upload madison_rl_agent.ipynb to Google Colab (colab.research.google.com)
2. Run Cell 1 -- installs dependencies
3. In Cell 2, set your Groq API key:
   GROQ_API_KEY = "your_groq_api_key_here"
   Get a free key at: console.groq.com
4. Runtime -- Run All
5. Training runs roughly 5 to 8 minutes on Colab CPU. No GPU needed.

No Groq key? Leave the key as-is -- the RL training still runs fully.
Only the LLM synthesis step is skipped.
...
| Groq (LLM synthesis) | OpenAI-compatible | Free signup only |
...
User Query + Topic Context
         |
         v
   Context Encoder (Topic to Index 0 to 6)
         |
    -----+-----
    |         |
UCB Bandit  REINFORCE
    |         |
    -----+-----
         |
   Source Selection (action)
    ...
    -----+-----+-----+-----+-----
    |         |     |     |     |
  Wiki     arXiv  Reddit Reddit DDG
                   /T     /S
         |
         v
   Reward Signal (quality + relevance)
         |
    -----+-----
    |         |
 UCB Update  REINFORCE Update
         |
         v
   Groq LLM Synthesis -- Intelligence Report
...
Welch t-test: UCB vs REINFORCE late phase
t = 1.82, p = 0.043 (significant at alpha = 0.05)

---

## Ethical Considerations

Source bias: Reward-maximizing agents may amplify source biases.
Mitigation: diversity constraints in action selection.

Misinformation: Reddit weighted lower in reward function and used
as supplementary signal only.

Transparency: All source selections and reward scores are logged
and visualizable.

Privacy: No personal data stored. System is stateless between sessions.

Boundaries: Queries only public APIs. All outputs carry a
